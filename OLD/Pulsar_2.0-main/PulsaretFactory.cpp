...
void PulsaretFactory::calculateWave(OwnedPulsaret& p1, OwnedPulsaret& p2)
{
    ...
    if (randWave.nextFloat() < waveRand.get() && waveIsSpread.get())
    {
        ...
        p1.pulsaretTable.setTable(waveNum / 100.f);
        p2.pulsaretTable.setTable(waveNum / 100.f);
    }
    else
    {
        p1.pulsaretTable.setTable(waveType.get() / 100.f);
        p2.pulsaretTable.setTable(waveType.get() / 100.f);
    }
...
{
    float f1 = form1; float f2 = form2;
    formIsHarm = foHarm;
    formIsHarm2 = foHarm2;
   

    smoothForm.setTargetValue(f1);
    smoothForm2.setTargetValue(f2);
}

void PulsaretFactory::setPulsaretDutyCycle(float width)
{

    float dutyCycle = pulsarPeriodInSamples * width;
    pulsaretSamples1 = dutyCycle;
    pulsaretSamples2 = dutyCycle;
